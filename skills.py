...
def get_sum_zero_pairs(input_list):
    ...
    zero_list = []
    zero_dict = {}
    halfway = len(input_list) / 2

    while i < halfway:

        for number in input_list[halfway:]:
            if (number + input_list[i] == 0):
                zero_dict[input_list[i], number] = None

        if input_list[i] == 0:
            zero_dict[input_list[i], input_list[i]] = None
        
        i += 1

    sum_zero_pairs = zero_dict.keys()
...
def remove_duplicates(words):
    ...
    duplicates_removed = no_duplicates.keys()
...
def get_pirate_talk(phrase):
    ...
    for key, value in replace_dict.iteritems():
        ...
        if key in phrase_list:
            key_location = phrase_list.index(key)
            phrase_list.remove(key)
            phrase_list.insert(key_location, value)
...
def adv_get_top_letter(input_string):
    ...
    tuples_list = character_dict.items()
    values_list = sorted(character_dict.values(), reverse=True)
    highest_count = values_list[0]

    for key, value in character_dict.iteritems():

        if value == highest_count:
            common_character_list.append(key)
            
    common_character_list.sort() 
